...
export default function ReportProfileButton({
  ...
  async function handleSubmitReport(e: React.FormEvent) {
    ...
    if (user.id === reportedUserId) {
      setMessage('You cannot report your own profile.')
      return
    }
  ...
  return (
    <>
      ...
      {open && (
        <div className="absolute inset-0 z-[70] flex items-center justify-center bg-black/75 px-4">
          <form
            onSubmit={handleSubmitReport}
            className="w-full max-w-sm rounded-3xl border border-white/10 bg-zinc-950 p-5 text-white shadow-2xl"
          >
            <h3 className="text-lg font-semibold">Report profile</h3>

            <p className="mt-2 text-sm text-zinc-400">
              Tell us why this profile should be reviewed.
            </p>
